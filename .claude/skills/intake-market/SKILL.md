...
4. **Callable flag** — binary: mobile/unknown phone → callable, landline/fixed/voip → not (kept for market stats). Record `pitch_angle` (no_website / has_website). Insert callable qualified into `prospects` as `status='qualified'`. No scoring/ranking in v1.
5. **Enrich** — one `enrichment-scout` agent per callable prospect (batch ~10 at a time, they use web search).
...
Report at the end: counts per stage (loaded / qualified / rejected+why / callable / enriched / sites built) and anything needing human review.
